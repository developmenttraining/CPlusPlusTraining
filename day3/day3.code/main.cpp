#include "insanx.h"
#include "superinsanx.h"
#include "robot.h"
#include "uzayinsanx.h"
...
void insanOgrenYasa(InsanX& ins) {
	ins.ogren();
}
...
void insanKalpAtisiBastir(const InsanX& ins) {
	cout << "Kalp atisi " << ins.kalpatisi << endl;
}
class KalpSayici {
public:
	InsanX insan{};
	...
	void privateInsanXeEris() {
		cout << "Kalp atisi " << insan.kalpatisi << endl;
	}
};
int main27() {
	InsanX ins{};
	insanKalpAtisiBastir(ins);//friend test
	return 0;
}
inline void donvesaydir() {
	for (int i{ 0 }; i < 10; ++i) {
...
int main26(){
 ...
 /*
					class içi		miras alýnan class içi		instance bolgesi
	public			OK				OK							OK
	protected		OK				OK							NO
	private			OK				NO							NO
 */
  
void testContClassParam(const SuperInsanX& s) {
	s.getIphoneVer();
	//s.setIphoneVer(11); izin vermez
}
int main25() {
	...
	const SuperInsanX nil{ };
	//nil.adi = 11;
	cout << nil.adi << endl;
	cout << nil.getIphoneVer() << endl;
...
//temel tipler, string ve enum pass by value
//class ve struct'lar pass by reference tercih tavsiye edilir.
void f(int a,TType t, const InsanX* ins) {
}
 
void kendiKendiniCagir() {
	static int kacKereCagrildiSay = 0;
	++kacKereCagrildiSay;
	int a;
	kendiKendiniCagir();
}
...
int main22() {
	...
	cout << InsanX::insanSayisi << endl;
	InsanX ali;
	cout << InsanX::insanSayisi << endl;
	InsanX veli{};
	cout << InsanX::insanSayisi << endl;
	InsanX* pIns = new InsanX{ "Cem", 20, 24042837422L };
	// pIns->kimlikNo = 3473253255L; yasak
	cout << pIns->kimlikNo << endl;
	cout << InsanX::insanSayisi << endl;
	delete pIns;
	cout << InsanX::insanSayisi << endl;
	InsanX ins[10]{};
...
int main21() {
	...
	int gun1 = PTESI;
	if (gun1 == PTESI)
		cout << "iyi haftalar." << endl;

	Gunler gun2 = PAZARTESI;
	if (gun2 == PAZARTESI)
		cout << "iyi haftalar." << endl;

	Days gun3 = Days::MONDAY;
	if (gun3 == Days::MONDAY)
		cout << "iyi haftalar." << endl;
	return 0;
}
...
/* Ödev
	Class Date
		...
		public
			 ...
			 Date(g, a, y)
			 goster()
			 nextday() // gunu 1 sonraki gune aktarýr
			 prevday() // gunu 1 sonraki gune aktarýr
			 addDay(int day)
			 addMonth(int day)
			 addYear(int day)

			 //Tum private variable set get olacak
			 //Limit kontrolleri yapýlacak.
